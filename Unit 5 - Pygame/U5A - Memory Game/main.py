...
class Main:
    def __init__(self):
        ...
        cursor_image = pygame.image.load(r"img\\cursor.png")
        ...
        self.__shuffle_button_rect = pygame.Rect(520, 275, 130, 35)
        self.__replay_button_rect = pygame.Rect(520, 325, 130, 35)
    ...
    def __shuffle_only(self):
        if self.__game_won or self.__game_lost:
            return
        current_tiles = [tile.face_up_img for tile in self.all_tiles]
        random.shuffle(current_tiles)
        self.__flipped_cards = []
        self.__waiting = False
        self.__build_grid_from_list(current_tiles)
    ...
    def __build_grid_from_list(self, tile_list):
        ...
        for row in range(4):
            for col in range(4):
                ...
                new_tile = Tile(tile_list[idx], x, y)
    ...
    def __load_music(self):
        try:
            pygame.mixer.music.load(r"sounds\\aria_math.ogg")
    ...
    def __handle_events(self):
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                elif self.__replay_button_rect.collidepoint(event.pos):
                    self.__reset_game()
                    if not pygame.mixer.music.get_busy():
                        self.__load_music()
                    return

                if not self.__waiting and not self.__game_won and not self.__game_lost:
    ...
    def __update(self):
        ...
        if not self.__game_won and not self.__game_lost and elapsed == 0:
    ...
    def __refresh(self):
        ...
        if self.__shuffle_button_rect.collidepoint(mouse_pos):
            shuffle_color = (60, 130, 200) # Bright Blue
        else:
            shuffle_color = (40, 90, 160)  # Dark Blue
        pygame.draw.rect(self.screen, shuffle_color, self.__shuffle_button_rect, border_radius=6)
        shuffle_txt = self.__font.render("Shuffle", True, (255, 255, 255))
        self.screen.blit(shuffle_txt, shuffle_txt.get_rect(center=self.__shuffle_button_rect.center))
        
        if self.__replay_button_rect.collidepoint(mouse_pos):
            btn_color = (40, 180, 120)
        else:
            btn_color = (30, 140, 90)
            
        pygame.draw.rect(self.screen, btn_color, self.__replay_button_rect, border_radius=6)
        btn_txt = self.__font.render("Replay", True, (255, 255, 255))
        self.screen.blit(btn_txt, btn_txt.get_rect(center=self.__replay_button_rect.center))
    ...
    def __draw_win_screen(self, elapsed):
        ...
        stats_surf = self.__font.render(f"Moves: {self.__moves}   Time: {45 - elapsed}s", True, (255, 255, 255))
...
class Tile(pygame.sprite.Sprite):
    def __init__(self, face_up, x, y):
        ...
        raw_back = pygame.image.load(r"img\\back.png").convert_alpha()
    ...
    def flip(self):
        ...
        if self.is_flip:
            raw_front = pygame.image.load(r"img\\" + self.face_up_img).convert_alpha()
